...
class ContextMenuTarget(Enum):
    ...
    FILE = "file"           # Einzelne Datei
    DIRECTORY = "directory" # Ordner
    BACKGROUND = "background"  # Ordner-Hintergrund
    DRIVE = "drive"         # Laufwerk


@dataclass
class ContextMenuItem:
    """Definition eines Kontextmenü-Eintrags."""
    name: str               # Interner Name (Registry-Key)
    label: str              # Angezeigter Text
    command: str            # Auszuführender Befehl
    icon: Optional[str] = None  # Icon-Pfad
    extensions: List[str] = None  # Dateierweiterungen (None = alle)
...
class RegistryManager(LoggerMixin):
    ...
    # Basis-Pfade für verschiedene Ziele
    PATHS = {
    ...
    def register_menu_item(self, item: ContextMenuItem, 
                           use_hkcu: bool = True) -> bool:
        """
        ...
        try:
            ...
            if item.extensions:
                # Für spezifische Dateitypen
                for ext in item.extensions:
                    self._register_for_extension(root, ext, item)
            else:
                # Für alle Dateien oder Ordner
                base_path = self.PATHS.get(item.target, self.PATHS[ContextMenuTarget.FILE])
    ...
    def _register_for_extension(self, root, ext: str, item: ContextMenuItem):
        """Registriert für eine Dateierweiterung."""
        # Erweiterung normalisieren
        if not ext.startswith('.'):
            ext = '.' + ext
        
        # Pfad für HKCU
        base_path = rf"Software\Classes\SystemFileAssociations\{ext}\shell"
    ...
    def unregister_menu_item(self, name: str, target: ContextMenuTarget = ContextMenuTarget.FILE,
                             extensions: List[str] = None, use_hkcu: bool = True) -> bool:
        """
    ...
    def _delete_key_recursive(self, root, path: str):
        ...
        try:
            ...
            # Dann den Schlüssel selbst
            winreg.DeleteKey(root, path)
        except FileNotFoundError:
            pass
    
    def register_cascading_menu(self, menu_name: str, label: str,
    ...
    def is_registered(self, name: str, target: ContextMenuTarget = ContextMenuTarget.FILE,
                      use_hkcu: bool = True) -> bool:
...
class DokuZenRegistry(LoggerMixin):
    ...
    def register_all(self) -> Dict[str, bool]:
        ...
        # PDF-Menü
        pdf_items = [
            ContextMenuItem("dz_pdf_open", "Mit DokuZen öffnen", 
                          self._build_command("open"), extensions=[".pdf"]),
            ContextMenuItem("dz_pdf_ocr", "OCR-Texterkennung",
                          self._build_command("ocr"), extensions=[".pdf"]),
            ContextMenuItem("dz_pdf_unlock", "PDF entsperren",
                          self._build_command("unlock"), extensions=[".pdf"]),
            ContextMenuItem("dz_pdf_redact", "Sensible Daten schwärzen",
                          self._build_command("redact"), extensions=[".pdf"]),
        ]
        
        results["pdf_menu"] = self._manager.register_cascading_menu(
            "DokuZen_PDF", "DokuZen Pro",
            pdf_items, extensions=[".pdf"]
        ) if hasattr(self._manager, 'register_cascading_menu') else False
        
        # Text-Spawner (alle Dateien)
        results["spawn_text"] = self._manager.register_menu_item(ContextMenuItem(
            "dz_spawn_text", "Als Text spawnen (DokuZen)",
        ...
        # Python-Menü
        py_items = [
            ContextMenuItem("dz_py_analyze", "Code analysieren",
                          self._build_command("analyze-code"), extensions=[".py"]),
            ContextMenuItem("dz_py_encoding", "Encoding prüfen/reparieren",
                          self._build_command("fix-encoding"), extensions=[".py"]),
        ]
        ...
        # Ordner-Hintergrund
        results["folder_open"] = self._manager.register_menu_item(ContextMenuItem(
            "dz_open_folder", "In DokuZen öffnen",
    ...
    def unregister_all(self) -> Dict[str, bool]:
        ...
        names = [
            "DokuZen_PDF", "dz_spawn_text", "dz_py_analyze",
            "dz_py_encoding", "dz_open_folder"
        ]
        
        for name in names:
            results[name] = self._manager.unregister_menu_item(name)
    ...
    def get_status(self) -> Dict[str, bool]:
        ...
        return {
            "pdf_menu": self._manager.is_registered("DokuZen_PDF"),
            "spawn_text": self._manager.is_registered("dz_spawn_text"),
            "py_analyze": self._manager.is_registered("dz_py_analyze"),
            "folder_open": self._manager.is_registered("dz_open_folder", ContextMenuTarget.BACKGROUND),
        }
...
def unregister_context_menus():
    ...
    registry = DokuZenRegistry()
    results = registry.unregister_all()
