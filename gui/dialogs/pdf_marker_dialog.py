...
class PDFMarkerDialog(QDialog, LoggerMixin):
    ...
    def _setup_ui(self):
        ...
        btn_merge_doc = QPushButton("Branch zusammenführen (Merge)...")
        btn_merge_doc.setToolTip("Führt Seiten des aktuellen Dokuments oder Auszüge mit weiteren Branch-PDFs zusammen.")
        btn_merge_doc.setAccessibleName("Branch zusammenführen (Merge)")
        btn_merge_doc.clicked.connect(self._merge_branches)
        action_layout.addWidget(btn_merge_doc)
    ...
    def _merge_branches(self):
        """Führt das aktuelle Dokument / Markierungen mit weiteren Branch-PDFs zusammen."""
        if not self._pdf_path or self._page_count <= 0:
            QMessageBox.warning(self, "Kein Dokument", "Bitte zuerst ein Dokument laden.")
            return

        start_dir = str(Path(self._pdf_path).parent)
        branch_files, _ = QFileDialog.getOpenFileNames(
            self,
            "Branch-/Auszug-PDFs zum Zusammenführen auswählen",
            start_dir,
            "PDF-Dateien (*.pdf)",
        )
        if not branch_files:
        ...
        marked_m = self._get_pages_by_marker('m')
        marked_k = self._get_pages_by_marker('k')

        current_pages = None
        if marked_m or marked_k:
            active_marker = 'm' if marked_m else 'k'
            marker_pages = marked_m if marked_m else marked_k
            marker_name = "Markierte (M)" if active_marker == 'm' else "Behalten (K)"

            msg = (
                f"Im aktuellen Dokument sind {len(marker_pages)} Seiten als '{marker_name}' markiert.\n\n"
                f"Möchten Sie nur diese markierten Seiten mit den gewählten Branches zusammenführen?\n"
                f"(Bei 'Nein' werden alle {self._page_count} Seiten verwendet)"
            )
            choice = QMessageBox.question(
                self,
                "Auswahl für Branch-Merge",
                msg,
            ...
            if choice == QMessageBox.StandardButton.Yes:
                current_pages = [p + 1 for p in sorted(marker_pages)]

        from core.pdf.branching import generate_branch_path, merge_branches
        default_target = generate_branch_path(self._pdf_path, "branch_merged")
        target_path, _ = QFileDialog.getSaveFileName(
            self,
            "Zusammengeführten Branch speichern",
            default_target,
            "PDF-Dateien (*.pdf)",
        )
        if not target_path:
        ...
        items = [(self._pdf_path, current_pages)]
        for bf in branch_files:
            items.append((bf, None))

        crop_mm = float(self._crop_margin_mm.value()) if self._crop_pages.isChecked() else None
        page_num = self._add_page_numbers.isChecked()

        try:
            success = merge_branches(
                items,
                target_path,
                crop_margin_mm=crop_mm,
                add_page_numbers=page_num,
            )
            if not success:
                QMessageBox.warning(self, "Merge fehlgeschlagen", "Es konnten keine Seiten zusammengeführt werden.")
                return
            ...
            reply = QMessageBox.question(
                self,
                "Branch-Merge erfolgreich",
                f"Branch erfolgreich erstellt:\n{target_path}\n({total_pages} Seiten)\n\n"
                f"Möchten Sie den zusammengeführten Branch jetzt im Marker öffnen?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                QMessageBox.StandardButton.No,
            )
            if reply == QMessageBox.StandardButton.Yes:
                self._load_pdf(target_path)
        except Exception as e:
            QMessageBox.critical(self, "Merge-Fehler", str(e))
